-- PROFO Aankoopbeheer - product toevoegen: Vuilniszakken 60 liter - 20 stuks - LDPE - Grijs.
...
-- Artikelnummer volgens aangeleverde bestandsnaam en 123schoon-productpagina: SDR00342.

with bron as (
  select
    'Vuilniszakken 60 liter - 20 stuks - LDPE - Grijs - 123schoon'::text as naam,
    'Schoonmaak'::text as categorie,
    '123schoon.nl'::text as leverancier,
    '{"artikelnummer":"SDR00342","url":"https://www.123schoon.nl/123schoon-Vuilniszakken-60-liter-20-stuks-LDPE-Grijs-123schoon-i7101.html"}'::text as leverancier_url,
    'Rol met 20 grijze LDPE-vuilniszakken van 60 liter. De zakken zijn scheurbestendig, recyclebaar en geschikt voor dagelijks afvalbeheer. Formaat per zak: 80 x 60 cm.'::text as omschrijving,
    'rol van 20 stuks'::text as eenheid,
    2.89::numeric(12, 2) as prijs_excl_btw,
    21::numeric(5, 2) as btw_percentage,
    1::integer as minimum_bestelhoeveelheid,
    '/assets/123schoon-vuilniszakken-60l-20-stuks-ldpe-grijs.jpg'::text as image_url,
    true::boolean as actief,
    530::integer as sort_order
),
target as (
  select product.id
...
where lower(naam) = lower('Vuilniszakken 60 liter - 20 stuks - LDPE - Grijs - 123schoon');
