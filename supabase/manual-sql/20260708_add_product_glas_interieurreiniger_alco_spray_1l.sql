-- PROFO Aankoopbeheer - product toevoegen: Glas- & interieurreiniger Alco spray 1 liter.
...
-- Artikelnummer volgens aangeleverde bestandsnaam en 123schoon-productpagina: SDR06055.

with bron as (
  select
    'Glas- & interieurreiniger Alco spray 1 liter - 123schoon huismerk'::text as naam,
    'Schoonmaak'::text as categorie,
    '123schoon.nl'::text as leverancier,
    '{"artikelnummer":"SDR06055","url":"https://www.123schoon.nl/123schoon-Glas-interieurreiniger-Alco-spray-1-liter-123schoon-huismerk-i13436.html"}'::text as leverancier_url,
    'Gebruiksklare glas- en interieurreiniger voor waterbestendige oppervlakken. Geschikt om oppervlakken streeploos schoon te maken door rechtstreeks te sprayen en af te nemen met een microvezeldoek. De reiniger hoeft niet verdund te worden. Inhoud: 1 liter. Gebruik volgens het etiket en raadpleeg de veiligheidsinformatie bij professioneel gebruik.'::text as omschrijving,
    'sprayflacon van 1 liter'::text as eenheid,
    7.99::numeric(12, 2) as prijs_excl_btw,
    21::numeric(5, 2) as btw_percentage,
    1::integer as minimum_bestelhoeveelheid,
    '/assets/123schoon-glas-interieurreiniger-alco-spray-1l.jpg'::text as image_url,
    true::boolean as actief,
    515::integer as sort_order
),
target as (
  select product.id
...
where lower(naam) = lower('Glas- & interieurreiniger Alco spray 1 liter - 123schoon huismerk');
