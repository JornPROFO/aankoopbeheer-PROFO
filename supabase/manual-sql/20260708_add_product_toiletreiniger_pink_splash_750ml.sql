-- PROFO Aankoopbeheer - product toevoegen: Toiletreiniger gel Pink Splash 750 ml.
...
-- Artikelnummer volgens aangeleverde bestandsnaam en 123schoon-productpagina: SDR06053.

with bron as (
  select
    'Toiletreiniger gel Pink Splash 750 ml - 123schoon huismerk'::text as naam,
    'Sanitair en hygiene'::text as categorie,
    '123schoon.nl'::text as leverancier,
    '{"artikelnummer":"SDR06053","url":"https://www.123schoon.nl/123schoon-Toiletreiniger-gel-Pink-Splash-750-ml-123schoon-huismerk-i13434.html"}'::text as leverancier_url,
    'Toiletreiniger gel Pink Splash voor het verwijderen van kalk- en urineaanslag in toiletpot of urinoir. Onverdund aanbrengen onder de rand en in het toilet, laten inwerken en naspoelen met water. Inhoud: 750 ml. Gebruik volgens het etiket en meng niet met andere reinigingsmiddelen.'::text as omschrijving,
    'fles van 750 ml'::text as eenheid,
    2.99::numeric(12, 2) as prijs_excl_btw,
    21::numeric(5, 2) as btw_percentage,
    1::integer as minimum_bestelhoeveelheid,
    '/assets/123schoon-toiletreiniger-pink-splash-750ml.jpg'::text as image_url,
...
target as (
  ...
  where lower(product.naam) in (
    lower(bron.naam),
    lower('WC-reiniger - product nog te bepalen')
  )
  order by
    case
...
where lower(naam) = lower('Toiletreiniger gel Pink Splash 750 ml - 123schoon huismerk');
